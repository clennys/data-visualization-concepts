...
from bokeh.models import (ColumnDataSource, NumeralTickFormatter, 
                          HoverTool, Label, Button, Slider, Text, ColorBar)
...
def create_df(year, city, market_cap_lower, main=True):
    ...
    data = us_company_map[['Symbol', 'City', 'x', 'y', f'Market Cap {year}', f'Employees {year}']]
    data = data.rename(columns={f'Market Cap {year}': 'Market_Cap', f'Employees {year}': 'Employees'})
    ...
    for element in data.to_dict('records'):
        if element['Market_Cap'] < market_cap_lower:
            element['Symbol'] = np.nan
            element['Market_Cap'] = np.nan
            element['Employees'] = np.nan
    ...
    if main:
        df = pd.DataFrame(data).groupby('City').agg(
            {'Symbol': 'count', 'x': 'mean', 'y': 'mean', 'Market_Cap': 'sum', 'Employees': 'sum'}
        )
    # For the subplot, find the companies in the selected `city`
    else:
        df = pd.DataFrame(data).loc[us_company_map['City'] == city]
        
    # Calculate 'circle_size' which is proportional to the log of 'Employees'
    df['circle_size'] = np.log(df['Employees']) * 2
...
def plot_city(main_df, tile_source):
    ...
    x_range = main_df['x'].min() - 200000, main_df['x'].max() + 200000
    y_range = main_df['y'].min() - 200000, main_df['y'].max() + 200000   
    p = figure(
        width=800, 
        height=600,
    ...
    color_mapper = log_cmap(
        ...
        palette=Turbo256,
        low=0,
        high=np.log(main_df['Market_Cap'].max())
    )
    c = p.circle(
        x='x', 
        y='y', 
    ...
    # Construct a color bar for the circle glyph with a log scale and number formatter
    color_bar = ColorBar(
        color_mapper=color_mapper['transform'],
        title='Market Cap in Billion USD',
    )
...
markt_max, markt_min = np.nanmax(us_company_map[f'Market Cap {year}']), np.nanmin(us_company_map[f'Market Cap {year}'])
emp_max, emp_min = np.nanmax(us_company_map[f'Employees {year}']), np.nanmin(us_company_map[f'Employees {year}'])
...
def plot_company(sub_df):
    ...
    x_range = (emp_min - 1000, emp_max + 1000)
    y_range = (markt_min - 1000, markt_max + 1000)
    p = figure(
        width=450,
        height=400, 
        min_border=20,
...
def tap_update(attr, old, new):
    if new:
        ...
        city = main_plot.renderers[1].data_source.selected['1d']['indices'][0]
...
main_plot.renderers[1].data_source.on_change('selected', tap_update)
...
slider = Slider(
    ...
    value=0,
    step=10,
    title="Market Cap Lower Bound (Billion USD)",
)
...
div = Div(
    text="""
    <h1>Circle Color: Market Cap in Billion USD</h1>
    <h1>Circle Size: Number of Employees</h1>
    <p>Move the slider to filter companies by the market cap.</p>
    <p>Click on play button below to show changes of Market</p>
    <p>Cap and Employees over the years.</p>
    """,
    width=800,
    height=200,
)
